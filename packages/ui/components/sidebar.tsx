...
function SidebarGroupAction({
	...
	return useRender({
		...
		props: mergeProps<"button">(
			{
				className: cn(
					"text-sidebar-foreground ring-sidebar-ring hover:bg-primary/5 hover:text-sidebar-accent-foreground absolute top-3.5 right-3 w-5 rounded-md p-0 focus-visible:ring-2 [&>svg]:size-4 flex aspect-square items-center justify-center outline-hidden transition-transform [&>svg]:shrink-0 after:absolute after:-inset-2 md:after:hidden group-data-[collapsible=icon]:hidden",
					className,
				),
			},
			props,
...
const sidebarMenuButtonVariants = cva(
	[
		// Layout
		"peer/menu-button group/menu-button flex w-full items-center gap-2 rounded-md p-2 text-left text-sm overflow-hidden",
		// Transitions
		"transition-[width,height,padding]",
		// Interactive states
		"hover:bg-primary/5 hover:text-sidebar-accent-foreground",
